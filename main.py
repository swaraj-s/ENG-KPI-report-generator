...
if st.sidebar.button("🚀 Run Effort Breakdown"):
    ...
    if not us_df.empty:
        ...
        us_df = pd.DataFrame(columns=["Assignee", "US_Effort"])
    ...
    if not dev_df.empty:
        ...
        dev_df = pd.DataFrame(columns=["Assignee", "DEV_Effort"])
    ...
    if not ps_df.empty:
        ...
        ps_df = pd.DataFrame(columns=["Assignee", "PS_Effort"])
    ...
    final_df["Dev Effort (US+QAB+QAR+Dev)"] = final_df["Bug_Effort"] + final_df["Reg_Effort"] + final_df["US_Effort"] + final_df["DEV_Effort"]
    final_df["Overall Effort"] = final_df["Dev Effort (US+QAB+QAR+Dev)"] + final_df["PS_Effort"]
    ...
    merged = people_df.merge(final_df, on="Name?", how="left")
